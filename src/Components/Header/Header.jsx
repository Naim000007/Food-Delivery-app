        ...
        <header className='fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16'>
            ...
            <div className='hidden md:flex w-full p-4 h-full items-center justify-between '>
                <Link to={'/'} className='flex items-center gap-2'>
                    ...
                    <p className='text-headingColor text-xl font-bold'>DishDash</p>
                </Link>
                        ...
                        <motion.img whileTap={{ scale: 0.6 }} src={user ? user.photoURL : avatar} alt="userimg" onClick={login} className='w-10   min-w-[40px] h-10 min-h-[40px] shadow-2xl cursor-pointer rounded-full' />
                        {
                            isMenu && (
                                <div className='w-40 bg-gray-100 shadow-xl roundedllg flex flex-col absolute top-12 right-0 px-4 py-2'>
                                    ...
                                    <p className=' flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base'  onClick={logout}>Log Out  <MdLogout /></p>
                                </div>
                            )
                        }
                    </div>
                    ...
                    <motion.img whileTap={{ scale: 0.6 }} src={user ? user.photoURL : avatar} alt="userimg" onClick={login} className='w-10   min-w-[40px] h-10 min-h-[40px] shadow-2xl cursor-pointer rounded-full' />
                    {
                        isMenu && (
                            <div className='w-40 bg-gray-100 shadow-xl roundedllg flex flex-col absolute top-12 right-0 px-4 py-2'>
                                ...
                                <p className='mb-2 p-2 rounded-md shadow-md flex items-center justify-center gap-2  cursor-pointer hover:bg-slate-400 transition-all duration-100 ease-in-out text-textColor text-base  bg-gray-200' onClick={logout}>Log Out  <MdLogout /></p>
                            </div>
                        )
                    }
                </div>
